 ...
 *
 */
public class MethodDeclarationTagProcessor extends AbstractAttributeTagProcessor{

	private static final String ATTR_NAME = "declarepk";
	private static final int PRECEDENCE = 10000;
	
	public MethodDeclarationTagProcessor(final String dialectPrefix) {
	...
	private String getJavaPrimaryKeys(EClass eClass) {
		...
		for (EAttribute attr:  eClass.getEAllAttributes()) {
			if (attr.getEAnnotation(AttributeAnnotations.ID)!=null) {
				...
				strb.append(TypeMapper.getJavaType(attr.getEType().getName())).append(" ").append(attr.getName());
			}
		}
