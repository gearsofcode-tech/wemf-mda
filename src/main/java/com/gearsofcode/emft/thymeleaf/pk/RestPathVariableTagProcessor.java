...
import com.gearsofcode.emft.metamodel.TypeMapper;
 ...
 *
 */
public class RestPathVariableTagProcessor extends AbstractAttributeTagProcessor{

	private static final String ATTR_NAME = "pathvarpk";
	private static final int PRECEDENCE = 10000;
	
	public RestPathVariableTagProcessor(final String dialectPrefix) {
	...
	private String getJavaPrimaryKeys(EClass eClass) {
		...
		for (EAttribute attr:  eClass.getEAllAttributes()) {
			if (attr.getEAnnotation(AttributeAnnotations.ID)!=null) {
				...
				strb.append("@PathVariable ").append(TypeMapper.getJavaType(attr.getEType().getName())).append(" ").append(attr.getName()).append(" ");
			}
		}
