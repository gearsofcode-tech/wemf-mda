 ...
 *
 */
public class JavaPrimaryKeyParameterTagProcessor extends AbstractAttributeTagProcessor{

	private static final String ATTR_NAME = "parampk";
	private static final int PRECEDENCE = 10000;
	
	public JavaPrimaryKeyParameterTagProcessor(final String dialectPrefix) {
	...
	private String getJavaPrimaryKeys(EClass eClass) {
		...
		for (EAttribute attr:  eClass.getEAllAttributes()) {
			if (attr.getEAnnotation(AttributeAnnotations.ID)!=null) {
				...
				strb.append(eClass.getName().toLowerCase()).append(".").append(getter(attr.getName())).append("()");
			}
		}
